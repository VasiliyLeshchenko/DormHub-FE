...
const TenantSearchInput = ({ onSelect, dormId }) => {
    ...
    const fetchData = async (searchText, pageNum) => {
        ...
        try {
            ...
            showAlert(`Произошла ошибка при загрузке жильцов: ${err.message}`, "danger");
        } finally {
            setLoading(false);
        }
    };
                    ...
                    >
                        <ListGroup>
                            {results.map((tenant) => (
                                ...
                                >
                                    {tenant.lastName} {tenant.firstName} {tenant.fatherName} ({EnumUtil.getFaculty(tenant.faculty)} {tenant.course? `,course ${tenant.course},`:null} {tenant.group})
                                </ListGroup.Item>
                            ))}
                        </ListGroup>
                    </InfiniteScroll>
