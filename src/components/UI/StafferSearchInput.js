...
const StafferSearchInput = ({ onSelect, dormId }) => {
    ...
    const fetchData = async (searchText, pageNum) => {
        ...
        try {
            ...
            showAlert(`Произошла ошибка при загрузке сотрудников: ${err.message}`, "danger");
        } finally {
            setLoading(false);
        }
    };
                    ...
                    >
                        <ListGroup>
                            {results.map((staffer) => (
                                ...
                                >
                                    {staffer.lastName} {staffer.firstName} {staffer.fatherName} ({staffer.dorm ? `${staffer.dorm.name}, ${staffer.dorm.address},` : ''} {EnumUtil.getPosition(staffer.position)})
                                </ListGroup.Item>
                            ))}
                        </ListGroup>
                    </InfiniteScroll>
...
export default StafferSearchInput;