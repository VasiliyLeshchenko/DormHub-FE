...
const ApartmentSearchInput = ({ onSelect, dormId}) => {
    ...
    const fetchData = async (searchText, pageNum) => {
        ...
        try {
            ...
            showAlert(`Произошла ошибка при загрузке помещений: ${err.message}`, "danger");
        } finally {
            setLoading(false);
        }
    };
                    ...
                    >
                        <ListGroup>
                            {results.map((apartment) => (
                                ...
                                >
                                    {apartment.number + apartment.suffix}, {EnumUtil.getApartmentType(apartment.type)}, {apartment.dorm.name}
                                </ListGroup.Item>
                            ))}
                        </ListGroup>
                    </InfiniteScroll>
...
export default ApartmentSearchInput;